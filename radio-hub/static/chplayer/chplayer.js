...
!(function() {
	 ...
	 * @since 2020.04.21
	 */
	var javascriptPath = (typeof static_url != 'undefined') ? static_url + '/chplayer/' : '';

	// 如果定义了HLS.js则替换调用本地文件
	var hlsUrl = (typeof wndt_hls_url != 'undefined') ? wndt_hls_url : javascriptPath + 'hls/hls.min.js';
		...
		getVideo: function() {
			...
			if (this.html5Video) { //如果支持HTML5-VIDEO则默认使用HTML5-VIDEO播放器
				...
				if (this.isM3u8) {
					...
					this.loadJs(hlsUrl, loadJsHandler);
				}
			...
			/**
			 *@since 2020.04.22
			 *构建时开始即添加缓冲图标，对部分加载慢的流媒体，用户体验更好
			 */
			if (true == thisTemp.vars.autoplay) {
		...
		*/
		...
		interFace: function() {
			...
			*/
			...
			//构建进度条内容结束
			this.getByElement(pauseCenterID).innerHTML = this.newCanvas(pauseCenterID, 0, 0); //构建中间暂停按钮
			...
			this.css([dlineID + '-la', dlineID + '-lb', dlineID + '-lc', dlineID + '-ra', dlineID + '-rb', dlineID + '-rc'], {
				...
				height: bHeight + 'px',
				overflow: 'hidden',
				borderLeft: '1px solid #500',
				borderRight: '1px solid #500',
